...
class ModulesLoader:
    ...
    def load_single(self, name):
        ...
        #print(f'{type(full_path)}: {full_path}')  # Print test
        self.printConfig(code, name)
    ...
    def printConfig(self, code, name):

        app_filename = self.get_full_path(name)
...
def create_ast(code):
    # Se añadió un print-test
    # print(f"\n{type(code)}\n{code}\n")
    ast_tree = ParentNodeTransformer().visit(ast.parse(code))
...
# se agregó un print para cada operador, en orden
def sort_operators(operators):
    aux = sorted(operators, key=lambda cls: cls.name())
    # for a in aux:
    #    print(a)
    return aux
